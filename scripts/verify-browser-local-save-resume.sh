#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WEB_DIR="$ROOT_DIR/web-runtime/build/web"
PROFILE="/tmp/mindustry-local-save-profile"
SAVE_DOM="/tmp/mindustry-local-save-dom.html"
CONTINUE_DOM="/tmp/mindustry-local-continue-dom.html"
PORT=8084

command -v google-chrome >/dev/null
[ -s "$WEB_DIR/index.html" ]
[ -s "$WEB_DIR/browser-storage.js" ]

cleanup(){
  if [ -n "${server_pid:-}" ]; then kill "$server_pid" 2>/dev/null || true; fi
}
trap cleanup EXIT

rm -rf "$PROFILE"
cd "$WEB_DIR"
python3 -m http.server "$PORT" --bind 127.0.0.1 >/tmp/mindustry-local-save-http.log 2>&1 &
server_pid=$!
for i in {1..30}; do
  if curl -fsS "http://127.0.0.1:$PORT/index.html" >/dev/null; then break; fi
  sleep 0.25
done

# First full Chrome process: enter the real packaged maze, accelerate one real
# survival wave, then write the fixed browser-owned current-v13 slot. Do not close
# the process until the IndexedDB flush marker confirms durable storage.
python3 "$ROOT_DIR/scripts/chrome-wait-dom.py" \
  --url "http://127.0.0.1:$PORT/index.html?lang=en&mindustryMapSmoke=maze&mindustrySaveSmoke=1" \
  --profile "$PROFILE" \
  --port 9232 \
  --timeout 45 \
  --require 'data-mindustry-web="ready"' \
  --require 'data-mindustry-storage="ready"' \
  --require 'data-mindustry-smoke-mode="production"' \
  --require 'data-mindustry-local-save-ui="ready"' \
  --require 'data-mindustry-local-continue-ui="ready"' \
  --require 'data-mindustry-local-map-test="maze"' \
  --require 'data-mindustry-local-map-wave-fired="yes"' \
  --require 'data-mindustry-local-map-save-smoke="armed"' \
  --require 'data-mindustry-local-map-save="ready"' \
  --require 'data-mindustry-local-map-save-slug="maze"' \
  --require 'data-mindustry-local-map-save-version="13"' \
  --require 'data-mindustry-local-save-state="saved"' \
  --require 'data-mindustry-local-save-version="13"' \
  --require 'data-mindustry-local-save-slot="available"' \
  --require 'data-mindustry-local-save-flush="ready"' \
  --require 'data-mindustry-local-map-loop="live"' \
  --require 'data-mindustry-network="local-only"' \
  --require 'data-mindustry-network-mode="singleplayer-only"' > "$SAVE_DOM"

grep -Eq 'data-mindustry-local-map-save-wave="[1-9][0-9]*"' "$SAVE_DOM"
grep -Eq 'data-mindustry-local-save-bytes="[1-9][0-9]{2,}"' "$SAVE_DOM"
grep -Eq 'data-mindustry-local-map-save-world="[1-9][0-9]*x[1-9][0-9]*"' "$SAVE_DOM"
saved_wave="$(grep -o 'data-mindustry-local-map-save-wave="[0-9]*"' "$SAVE_DOM" | head -1 | sed -E 's/.*="([0-9]+)"/\1/')"

# Second completely new Chrome process, same origin/profile: browser-storage.js
# hydrates the MSAV before TeaVM starts. Continue must resolve the saved map back to
# the pinned built-in catalog, restore the world with SaveIO.load, and execute 3+
# real production frames without logic.play() resetting the saved wave.
python3 "$ROOT_DIR/scripts/chrome-wait-dom.py" \
  --url "http://127.0.0.1:$PORT/index.html?lang=en&mindustryContinueSmoke=1" \
  --profile "$PROFILE" \
  --port 9233 \
  --timeout 45 \
  --require 'data-mindustry-web="ready"' \
  --require 'data-mindustry-storage="ready"' \
  --require 'data-mindustry-smoke-mode="production"' \
  --require 'data-mindustry-local-save-slot="available"' \
  --require 'data-mindustry-local-save-ui="ready"' \
  --require 'data-mindustry-local-continue-ui="ready"' \
  --require 'data-mindustry-local-continue-slot="available"' \
  --require 'data-mindustry-local-continue-smoke="requested"' \
  --require 'data-mindustry-local-save-load="ready"' \
  --require 'data-mindustry-local-save-load-version="13"' \
  --require 'data-mindustry-local-continue="ready"' \
  --require 'data-mindustry-local-continue-slug="maze"' \
  --require 'data-mindustry-local-continue-version="13"' \
  --require 'data-mindustry-local-map-state="playing"' \
  --require 'data-mindustry-local-map-slug="maze"' \
  --require 'data-mindustry-local-map-player="added"' \
  --require 'data-mindustry-local-map-loop="live"' \
  --require 'data-mindustry-local-map-module-order="logic-pathfinding-control-renderer-ui"' \
  --require 'data-mindustry-network="local-only"' \
  --require 'data-mindustry-network-mode="singleplayer-only"' > "$CONTINUE_DOM"

grep -Eq 'data-mindustry-local-map-frames="([3-9]|[1-9][0-9]+)"' "$CONTINUE_DOM"
grep -Eq 'data-mindustry-local-map-update-id="[1-9][0-9]*"' "$CONTINUE_DOM"
grep -Eq 'data-mindustry-local-continue-world="[1-9][0-9]*x[1-9][0-9]*"' "$CONTINUE_DOM"
continued_wave="$(grep -o 'data-mindustry-local-continue-wave="[0-9]*"' "$CONTINUE_DOM" | head -1 | sed -E 's/.*="([0-9]+)"/\1/')"
test -n "$saved_wave"
test "$continued_wave" = "$saved_wave"

echo 'Browser local Save/Continue: real maze + wave -> v13 MSAV -> IndexedDB flush -> full Chrome restart -> Continue -> 3+ restored production frames PASS'
