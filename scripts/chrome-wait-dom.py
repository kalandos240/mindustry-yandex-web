...
def evaluate(ws: WebSocket, message_id: int, expression: str) -> str:
    ws.send_text(json.dumps({"id": message_id, "method": "Runtime.evaluate", "params": {"expression": expression, "returnByValue": True}}))
    while True:
        ...
        return str(payload.get("result", {}).get("result", {}).get("value", ""))
...
def main() -> int:
    ...
    command = [args.chrome, "--headless=new", "--no-sandbox", "--disable-dev-shm-usage", "--use-gl=angle", "--use-angle=swiftshader", "--enable-unsafe-swiftshader", "--remote-allow-origins=*", f"--remote-debugging-port={args.port}", f"--user-data-dir={profile}", args.url]
    ...
    try:
        ...
        sys.stderr.write((last_html[start:end + 1] if start >= 0 and end >= start else last_html[:1000]) + "\n")
        return 1
    finally:
        if ws is not None:
            ws.close()
