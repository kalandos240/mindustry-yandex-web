...
run_campaign_restart(){
  ...
  # First Chrome process: start stock Ground Zero, execute real campaign frames,
  # write a current-v13 sector checkpoint and explicitly flush IndexedDB.
  ...
  # hydrates sector-serpulo-170.msav before TeaVM starts. BrowserSaves must reindex
  # the same sector slot and SaveSlot.load(sector context) must resume it unchanged.
  python3 "$ROOT_DIR/scripts/chrome-wait-dom.py" \
  ...
  echo "Browser campaign Save/Resume ($label): Ground Zero -> 3+ frames -> stock sector checkpoint -> IndexedDB flush -> full Chrome restart -> SaveSlot.load(sector context) -> identical wave/tick/bytes -> 3+ campaign frames PASS"
}
